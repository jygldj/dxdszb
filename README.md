...
| 线路 | CDN 地址 | 上游源 | 说明 |
|---|---|---|---|
| 直播源1（库老） | https://dxdszb.pages.dev/live.m3u | https://live.445569.xyz/live.m3u | IPTV 直播源，每 12 小时镜像，剔除 ✈️TG频道@stymei / 抖音 / 快手 三组 |
...
- **live.m3u**：拉取上游（带重试 + `#EXTM3U` 校验）→ 按 `group-title` **整组剔除**
  三组（✈️TG频道@stymei / 抖音 / 快手）→ 写文件。上游拉取失败时保留上一份不动。
