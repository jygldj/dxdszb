...
# 为了简化，这里直接硬编码（实际可从文件读取，但插件环境通常不支持文件读取，故硬编码）
YOUTUBE_CLASSES = [
...
# 以下为原代码中保留的辅助字典（用于构建关键词，可能不再需要，但保留以防万一）
CATEGORY_QUERY = {}  # 留空，因为我们直接使用分类名称作为关键词
CATEGORY_ALIASES = {} # 留空
# ==================== 核心提取类 ====================
class YouTubeLite:
    """普通视频提取（原 youtube2.py）"""
...
class YouTubeLiveLite:
    ...
    # 复用通用的 JSON 提取（与 YouTubeLite 共享，但为了方便我们直接引用方法，或者重新定义）
    # 这里直接复制 YouTubeLite 中的几个方法，也可以调用实例方法，但为了独立，我们重新定义。
    def _extract_ytcfg(self, text):
        m = re.search(r'ytcfg\.set\s*\(\s*({.+?})\s*\)\s*;', text, re.S)
        if not m:
            return None
...
class Spider(Spider):
    ...
    # ---------- 以下方法与原合并版本完全相同，此处仅作占位 ----------
    def _is_live_category(self, cid):
        """判断分类是否为直播（根据 type_id 是否包含 'Live' 或 '直播'）"""
        cid_lower = cid.lower()
    ...
    def _build_video_keyword(self, cid, filters=None):
        ...
        if cid.startswith('LIST:'):
            ...
            # 如果频道名以 @ 开头，则使用 channel 搜索语法；否则直接用频道名
            terms = []
            for ch in channels:
                if ch.startswith('@'):
    ...
    # 搜索方法（视频和直播）与原代码一致，此处省略具体实现（实际需保留原有代码）
    # 以下为占位，实际需复制原 youtube1.py 中的对应方法
    ...
    def _search_live_page(self, key, page=1):
        page = max(1, int(page or 1))
        # 直播搜索使用 sp=EgJAAQ%253D%253D 过滤直播
        # 简单实现：复用原 youtubelive 的搜索方法，但需要改造为分页
        # 由于原 youtubelive 没有实现分页，我们借用视频搜索的框架，但搜索 URL 加上 sp
    ...
    def _fetch_live_search_first_page(self, key):
        ...
        return {
            ...
            'referer': search_url,
            'pages': [self._extract_live_videos_from_api(data, 30)],   # 标记直播
    ...
    def _extract_live_videos_from_api(self, data, limit=30):
        # 与普通视频提取类似，但将 vod_remarks 设为 "直播"
        videos = []
        seen = set()
        def scan(obj):
            if len(videos) >= limit:
